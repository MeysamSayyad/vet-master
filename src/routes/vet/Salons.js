...
<div className="flex flex-row">
  <div className='flex flex-col items-center'>
    ...
    {data.map(i=>
      <Link to={`userId/${id}/salonId/${i.id}`} onClick={setsalonName(`${i.name}`)} style={salonId==i.id ? {borderWidth:'1.5px',borderColor:'grey',boxShadow:'0px 2px 3px 1px grey'}:undefined} className="border px-8 py-3 rounded-xl m-2 bg-slate-50 shadow w-[30vw] text-center text-lg ">
        {i.name}
      </Link>
    )}
  </div>
